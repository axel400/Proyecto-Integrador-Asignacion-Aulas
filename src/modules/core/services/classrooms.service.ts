...
import { PaginationDto, CreateClassroomDto, FilterClassroomDto, UpdateClassroomDto } from '@core/dto';
...
export class ClassroomsService {
  ...
  async update(id: number, payload: UpdateClassroomDto): Promise<ServiceResponseHttpModel> {
  ...
  async remove(id: number): Promise<ServiceResponseHttpModel> {
    ...
    const classroomDeleted = await this.classroomRepository.softRemove(classroom);

    return { data: classroomDeleted };
  }

  async removeAll(payload: ClassroomEntity[]): Promise<ServiceResponseHttpModel> {
